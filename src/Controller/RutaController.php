...
final class RutaController extends AbstractController
{
    ...
    #[Route('photos/{id}', name: 'app_ruta_fotos', methods: ['GET'])]
    public function fotosRuta(RutaRepository $rutaRepository, Ruta $ruta): Response
    {
        return $this->render('ruta/fotosruta.html.twig', [
            'fotos' => $rutaRepository->getFotosRutaByRuta($ruta),
        ]);
    }
    ...
    public function delete(Request $request, Ruta $rutum, EntityManagerInterface $entityManager): Response
    {
        if ($this->isCsrfTokenValid('delete'.$rutum->getId(), $request->getPayload()->getString('_token'))) {
            ...
            $imageName = $rutum->getImageName();
            $uploadDirectory = $this->getParameter('kernel.project_dir') . '/public/uploads/images/routes';
